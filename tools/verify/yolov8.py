...
``tools/export/yolov8.py`` already verified the graph against the torch model to a hundredth of a
...
def vendor_detections(variant: str, images: dict) -> dict[str, tuple]:
    ...
    detector = Detector(resolve("yolov8", variant, "torch-fp32"), device="cpu")
    found = {}
...
def mozo_detections(variant: str, images: dict, runtime: str) -> dict[str, tuple]:
    ...
    """
    model = mozo.ModelManager().get_model("yolov8", variant, device="cpu", runtime=runtime)
    found = {}
    for name, pixels in images.items():
...
def main() -> int:
    ...
    print(f"yolov8/{args.variant}, {len(images)} image(s), threshold {THRESHOLD}")
    published = artifacts("yolov8", args.variant)
