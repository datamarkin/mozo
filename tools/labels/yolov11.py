#!/usr/bin/env python3
"""Write the class vocabulary for each YOLO11 variant into the local ``weights/`` tree.
...
have to be published beside it or that runtime can name nothing it finds.
    ...
    python tools/labels/yolov11.py                 # every fetched variant
    python tools/labels/yolov11.py nano small
...
YOLO11 numbers its classes contiguously from 0, unlike RF-DETR's original sparse COCO ids, and
is read from its own checkpoint rather than copied from the YOLOv8 vocabulary it happens to
match -- a fine-tuned checkpoint would not.
...
def main() -> int:
    ...
    family = args.weights_dir / "yolov11"
    wanted = args.variants or sorted(p.name for p in family.iterdir() if p.is_dir())
    if not wanted:
        raise SystemExit(f"nothing fetched under {family}. Run tools/fetch/yolov11.py first.")
