...
There is no CoreML artifact for this family, unlike YOLOv8. The attention block Apple's Metal
graph compiler cannot place makes conversion abort the process rather than raise, and the CoreML
...
from ..vendors.yolov11_deploy import Detector, detect
...
class YOLOv11Predictor:
    """One loaded YOLO11 variant, ready to run.
        ...
        *,
        ...
        if checkpoint_path is None:
            self.runtime = select_runtime(
                self.device, artifacts("yolov11", variant, revision=revision), runtime)
        else:
        ...
        if self.runtime.startswith("torch"):
            weights = (Path(checkpoint_path) if checkpoint_path
                       else resolve("yolov11", variant, self.runtime, revision=revision))
            self._detector = Detector(weights, device=self.device)
            self.imgsz = self._detector.imgsz
            names: Any = self._detector.names
        else:
            artifact = resolve("yolov11", variant, self.runtime, revision=revision)
        ...
        self._labels = labels_for(
            "yolov11", variant, caller=labels, checkpoint=names,
        ...
        print(f"YOLO11 {variant} ready on {self.device} via {self.runtime} at {self.imgsz}px.")
    ...
    def _forward(self, batch: torch.Tensor) -> torch.Tensor:
        ...
        :func:`~mozo.vendors.yolov11_deploy.detect`.
        """
