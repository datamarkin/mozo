...
Deployment-only YOLOv8 detection. Apache-2.0 code that does not import, depend on, or contain any
...
detector = Detector("yolov8n.pt")                      # imgsz=640, device="cpu", fuse_norm=True
...
Most users reach this through `mozo.adapters.yolov8`, which resolves the weights, picks the
...
kernel sizes, strides, paddings, the batch-norm epsilon, split widths, the number of DFL bins, the
per-level strides and the class names.
...
`Detector.raw` returns the undecoded head output `(1, 4 + classes, anchors)` together with the
letterbox placement, which is the seam mozo runs a non-torch graph through: feed `letterbox`'s
batch to any runtime, then hand its output to `suppress` and `to_original`. Both runtimes share
that pre- and post-processing, so they cannot drift apart.

## Supported

- Detection checkpoints in the YOLOv8 format (`task: detect`). Anything else is refused by name.
- The module classes found in such a checkpoint, and only those: `Conv`, `Bottleneck`, `C2f`,
  `SPPF`, `Concat`, `DFL`, `Detect`, plus the `torch.nn` leaves `Conv2d`, `BatchNorm2d`, `SiLU`,
  `MaxPool2d`, `Upsample`, `Sequential` and `ModuleList`.
...
from them — an ONNX graph, a CoreML package — contains the weights and carries the same terms.
mozo publishes the licence text and a NOTICE beside every checkpoint. If you serve predictions
from these weights over a network, AGPL-3.0 section 13 places obligations on you.
