...
Vendored from RF-DETR, which in turn adapted the HuggingFace Transformers DINOv2-with-Registers implementation
and added windowed attention.  Every leaf module is defined here; the Transformers base classes this file used to
inherit from are reimplemented in :mod:`._hf_compat`, so the deployment install needs no ``transformers``.

Removed relative to the RF-DETR original: the plain ``...Model`` and ``...ForImageClassification`` heads (neither is
reachable from detection, segmentation, or keypoint inference), head pruning, and the Transformers doc-string
decorators.  ``set_attn_implementation`` moved from the deleted ``...Model`` onto the shared base class so the
backbone keeps its eager/sdpa switch.
"""
...
class WindowedDinov2WithRegistersConfig(PretrainedConfig):
    r"""
    ...
    >>> from rfdetr_deploy.models.backbone.dinov2_with_windowed_attn import WindowedDinov2WithRegistersConfig
...
class WindowedDinov2WithRegistersPreTrainedModel(PreTrainedModel):  # type: ignore[no-untyped-call]
    ...
    def set_attn_implementation(self, attn_implementation: str) -> None:  # type: ignore[override]
        ...
        Example::
            ...
            >>> from rfdetr_deploy.models.backbone.dinov2_with_windowed_attn import (
