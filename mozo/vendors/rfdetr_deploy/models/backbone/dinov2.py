...
class DinoV2(nn.Module):
    def __init__(
        ...
        if not use_windowed_attn:
            # Upstream builds this branch with transformers' AutoBackbone, which downloads DINOv2 weights from the
            # HuggingFace Hub. Every released RF-DETR variant uses windowed attention and carries its backbone weights
            # in its own checkpoint, so the branch is unreachable here and is not worth a `transformers` dependency.
            raise ValueError(
                f"encoder {name!r} requests non-windowed attention, which this deployment-only build does not "
                "support: it would require downloading DINOv2 weights from the HuggingFace Hub. Use a windowed "
                "encoder (any released RF-DETR variant)."
            )
        else:
