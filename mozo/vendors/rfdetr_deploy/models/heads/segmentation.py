...
from ...utilities.tensors import _bilinear_grid_sample
...
def point_sample(input: Tensor, point_coords: Tensor, **kwargs: Any) -> Tensor:
    """A wrapper around ``_bilinear_grid_sample`` to support 3D point_coords tensors.
    ...
    Returns:
        A tensor of shape (N, C, P) or (N, C, Hgrid, Wgrid) that contains
            ...
            interpolation from `input` the same way as ``_bilinear_grid_sample``.
    """
    add_dim = False
