...
from .build import build_network, fuse_conv_bn
...
def detect(
    ...
    suppress the overlaps, map the survivors back to source pixels. A torch module, an ONNX
    session and a CoreML package differ only in *forward*, so all of them go through one pre- and
    post-processing path and cannot drift apart -- which is the property mozo publishes three
    artifacts on.
...
class Detector:
    ...
    def __init__(
        ...
        self.network = build_network(load_checkpoint(os.fspath(checkpoint)))
        if fuse_norm:
            fuse_conv_bn(self.network)
